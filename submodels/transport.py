...
def dilute_solution(state_1, state_2, sep):
    # If the porosity of either volume is zero, then the flux is automatically 
    # zero. Otherwise, calculate the flux using Poisson-Nernst-Planck:
    if min(state_1['microstructure'], state_2['microstructure']) < 1e-5:
        N_k_elyte = np.zeros_like(state_1['C_k'])
        i_io = 0
    else:
        dy_inv = 1. / (state_1['dy'] + state_2['dy'])

        C_k_int = ((state_1['dy'] * state_1['C_k'] + state_2['dy'] 
            * state_2['C_k']) * dy_inv)
